    ...
    function checkScrollAnim() {
      ...
      for (i = 0; i < numPinned; i++) {
        ...
        if (pinObj.state != 'PINNED') {
          ...
          startPoint = pinObj.spacer ? superscrollorama.settings.isVertical ? pinObj.spacer.offset().top : pinObj.spacer.offset().left : superscrollorama.settings.isVertical ? el.offset().top : el.offset().left;
          ...
          if (currScrollPoint > startPoint && currScrollPoint < endPoint) {
            ...
            if (pinObj.origPositionVal === 'auto') pinObj.origPositionVal = 0;
            else pinObj.origPositionVal = parseInt(pinObj.origPositionVal, 10);

            // change to fixed position
            el.css('position', 'fixed');
            if (superscrollorama.settings.isVertical) el.css('top', -pinObj.offset);
            else el.css('left', -pinObj.offset);

            pinObj.pinStart = startPoint;
            pinObj.pinEnd = endPoint;

            if (pinObj.spacer) pinObj.spacer.css('height', pinObj.dur + elHeight);

            if (pinObj.onPin) pinObj.onPin();
          }
          ...
          if (currScrollPoint < pinObj.pinStart || currScrollPoint > pinObj.pinEnd) {
            ...
            if (superscrollorama.settings.isVertical) el.css('top', pinObj.origPositionVal);
            else el.css('left', pinObj.origPositionVal);

            if (pinObj.spacer) pinObj.spacer.css('height', currScrollPoint < pinObj.pinStart ? 0 : pinObj.dur);

            if (pinObj.onUnpin) pinObj.onUnpin();
          } else if (pinObj.anim) {
            // do animation
